...
export const rasterizeTriangle = (
  ...
  const ndcToScreen = (v: Vec4) => {
    return vec4(
      v[0] * (canvas.context.canvas.width / 4), // (v[0] + 1) / 2 * canvas.context.canvas.width,
      -v[1] * (canvas.context.canvas.height / 4), // (v[1] + 1) / 2 * canvas.context.canvas.height,
      v[2],
      v[3],
    );
  };
  ...
  const cameraLocation = transformVec4(perspectiveMatrix, vec4(0, 0, 0, 1));
  ...
  for (let x = Math.round(topLeft[0]); x <= bottomRight[0]; x += 1) {
    for (let y = Math.round(topLeft[1]); y <= bottomRight[1]; y += 1) {
      ...
      if (pixelCoord[2] >= -1 && pixelCoord[2] <= 2 && pixelCoord[2] < currentDepth) {
        ...
        let red = 0; //Math.floor(u * 255);
        let green = 0; // Math.floor(v * 255);
        let blue = 0; // Math.floor(0 * 255);
        let alpha = 255;
